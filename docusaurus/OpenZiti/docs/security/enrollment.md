...
[3rd Party CA](authentication/third-party-cas.md). When creating an identity the `id` of the target
[3rd Party CA](authentication/third-party-cas.md) is specified.
...
Auto CA enrollment allows a [3rd Party CA](authentication/third-party-cas.md) to have clients enroll with a
Ziti network without first creating an identity or distributing a JWT enrollment token. Create a
[3rd Party CA](authentication/third-party-cas.md) and ensure that `isAutoCaEnrollmentEnabled` is set to `true`.

The name of enrolling clients is controlled by the `identityNameFormat` of the [3rd Party CA](authentication/third-party-cas.md).
The format support a number of replacement strings:

- `[caName]` - the Ziti `name` of the [3rd Party CA](authentication/third-party-cas.md) that validates the enrolling certificate
- `[caId]` - the Ziti `id` of the [3rd Party CA](authentication/third-party-cas.md) that validates the enrolling certificate
...
Ziti. If the client's certificate was issued by a [3rd Party CA](authentication/third-party-cas.md), the
